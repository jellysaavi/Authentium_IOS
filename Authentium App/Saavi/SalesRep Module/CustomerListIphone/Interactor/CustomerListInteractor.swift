...
class CustomerListInteractor: NSObject, CustomerListViewInteractorInputProtocol
{
    var presenter : CustomerListViewPresenterProtocol?
    var dataManager : CustomerListDataManagerProtocol?

    
    func getCustomerFeatures(customerID:String) {
        self.dataManager?.getCustomerFeatures(customerID:customerID)
    }
...
extension CustomerListInteractor : CustomerListViewInteractorOutputProtocol
{
    func handleResponseFromAPI(response : Any)
    {
        ...
        //self.getCustomerFeatures(customerID:)
