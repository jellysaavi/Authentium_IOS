...
//  Saavi
//
//  Created by goMad Infotech on 25/02/19.
...
class ChildListViewController: UIViewController,CustomerListViewProtocol {
    
    //MARK: - - Outlets
    static let childListStoryboard = "ChildListViewController"
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        UserInfo.shared.customerID = UserInfo.shared.parentId
    ...
    fileprivate func commonInit(){
        ...
        attributedString = NSMutableAttributedString(attributedString: self.setAttributedText(text: "Name: ", fontName: "SFUIDisplay-Regular", color: .gray))
    ...
    fileprivate func setDefaultNavigation() -> Void
    {
        ...
        let lblTitle = UIButton(type: .custom)
        lblTitle.setTitle("Select Customer", for: .normal)
        lblTitle.setTitleColor(UIColor.baseBlueColor(), for: .normal)
        lblTitle.tintColor = UIColor.baseBlueColor()
        lblTitle.frame = CGRect(x: 0, y: 44, width: 34, height: 44)
        lblTitle.imageView?.contentMode = .scaleAspectFit
        
        let barLogoutBtn = UIBarButtonItem(customView: logoutBtn)
        let barTitleLbl = UIBarButtonItem(customView: lblTitle)
        self.navigationItem.leftBarButtonItems = [barLogoutBtn,barTitleLbl]
    ...
    func getRepCustomerList(searchText:String){
        ...
        SyncEngine.sharedInstance.sendPostRequestToServer(dictionary: requestParameters as! Dictionary<String, Any>, strURL: serviceURL) { (response : Any) in
            if (response as? Dictionary<String,Any>) != nil
            {
                ...
                if (response is Dictionary<String,Any>) && (response as! Dictionary<String,Any>).keyExists(key: "ChildCustomers"), let customerListArray = (response as! Dictionary<String,Any>)["ChildCustomers"] as? Array<Dictionary<String,Any>>
                {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.wireframe == nil {
            ...
            DispatchQueue.main.async {
                ...
                {
                    let customerDetail  = self.customerList[indexPath.row]
                    UserInfo.shared.customerID = "\(customerDetail["CustomerID"] as? Int ?? 0)"
...
extension ChildListViewController:UISearchBarDelegate{
...
import Foundation
import UIKit
class ChildListWireframe{
    
    class func makeChildListAsRoot(){
        
        let loginWireframe = LoginWireFrame()
        loginWireframe.makeRootViewController(onWindow: UIApplication.shared.keyWindow!, isShowChild: true)
